...
import { getActivityLog, type Activity } from '@/lib/activity-log';
...
export default function AdminManagementPage() {
    ...
    const getIconForType = (type: Activity['type']) => {
        switch (type) {
            case 'USER_LOGIN': return <UserCheck className="h-4 w-4 text-primary" />;
            case 'FILE_UPLOAD': return <UploadCloud className="h-4 w-4 text-primary" />;
            case 'ADMIN_LOGIN': return <UserCheck className="h-4 w-4 text-primary" />;
            case 'USER_REGISTER': return <UserPlus className="h-4 w-4 text-primary" />;
            case 'SYSTEM_INIT': return <RefreshCw className="h-4 w-4 text-primary" />;
                ...
                <CardContent className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 grid md:grid-cols-2 gap-8">
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between p-3 bg-card rounded-md border border-border">
                                    ...
                                    <span className="font-bold text-lg">{mockStats.totalUsers}</span>
                                </div>
                                <div className="flex items-center justify-between p-3 bg-card rounded-md border border-border">
                                    <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center justify-between p-3 bg-card rounded-md border border-border">
                                    ...
                                    <span className="font-bold text-lg">{mockStats.activeSessions}</span>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div id="activity-log" className="space-y-3 h-48 overflow-y-auto pr-2">
                                {activityLog.map((activity, index) => (
                                    <div key={index} className="flex items-center p-3 bg-card rounded-md border border-border text-sm animate-in fade-in-50">
                                        {getIconForType(activity.type)}
                                        <span className="ml-3 flex-1">{activity.text}</span>
